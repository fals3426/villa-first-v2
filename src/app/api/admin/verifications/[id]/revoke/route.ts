...
const revokeSchema = z.object({
  reason: z.string().min(1, 'La raison de révocation est obligatoire'),
});
...
export async function POST(
  ...
  try {
    ...
    try {
      const updated = await verificationService.revokeVerification(
        id,
        session.user.id,
      ...
      return NextResponse.json({
        data: { status: 'revoked', verificationRequest: updated },
      ...
      if (error instanceof Error) {
          ...
          return NextResponse.json(
            {
              error: {
                ...
                message:
                  'Cette vérification ne peut pas être révoquée dans son état actuel',
              },
            },
            { status: 409 }
          );
        ...
        if (error.message === 'REVOCATION_REASON_REQUIRED') {
          return NextResponse.json(
            {
              error: {
                code: 'VALIDATION_ERROR',
